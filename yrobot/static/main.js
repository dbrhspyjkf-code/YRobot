...
function renderMic(mic) {
  ...
  audioStatus.classList.toggle("voiced", Boolean(mic.voiced));
  audioStatus.textContent = mic.voiced ? "检测到人声" : "运行中";
  const stamp = new Date().toLocaleTimeString("zh-CN", { hour12: false });
  audioMicNote.textContent = `实时电平 · RMS ${(mic.rms || 0).toFixed(4)} · ${stamp}`;
}
