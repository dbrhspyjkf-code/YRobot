...
public final class RobotSession {
    ...
    @discardableResult
    public func connect(to endpoint: RobotEndpoint, hardwareId: String? = nil, robotName: String? = nil) async -> Bool {
        ...
        do {
            ...
            availability.daemon = .unavailable(reason: String(describing: error), since: .now)
        }
        do {
            self.yrobotClient = APIClient(baseURL: try endpoint.yrobotBaseURL(), session: session)
        } catch {
            availability.yrobot = .unavailable(reason: String(describing: error), since: .now)
        }
        ...
        // have a stable identifier (Bonjour name + hardware id).
        if hardwareId != nil, case .bonjourName(let name) = endpoint.host {
        ...
        return availability.daemon == .available || availability.yrobot == .available
    }
