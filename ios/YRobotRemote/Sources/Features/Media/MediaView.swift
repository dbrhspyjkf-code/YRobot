...
struct MediaView: View {
    ...
    private var volumeSection: some View {
        Section("Speaker volume") {
            ...
            // Mute is a client-side semantic: percent == 0 means muted. The
            // dashboard's Mute button just sets volume to 0 and remembers
            // the last non-zero value for unmute; there is no separate
            // /api/volume/mute endpoint on the server.
    ...
    private var micSection: some View {
        Section {
            // Toggle is always visible so the operator can recover
            // from a paused / MIC-disabled state. When the live
            // value is unknown (no status yet) we show the toggle as
            // off, since the Dashboard defaults to mic-disabled.
            let inputEnabled = session.status?.status.audio.input_enabled ?? false
            HStack {
                Text("MIC enable")
                Spacer()
                Toggle("", isOn: Binding(
            ...
            if let mic = session.status?.status.audio.mic {
                ...
                ProgressView(value: Double(pct), total: 100) {
                    Text("Reachy mic level")
                }
                .accessibilityLabel("Reachy microphone level \(pct) percent")
                LabeledContent("dB", value: String(format: "%.0f", mic.level_db))
                LabeledContent("Voice active", value: mic.voiced ? "yes" : "no")
            } else if session.status != nil {
                Text("Reachy mic data unavailable while paused")
                    .foregroundStyle(.secondary)
                    .font(.footnote)
            }
            if let micError {
            ...
            Text("Reachy microphone")
        } footer: {
            Text("This shows the level the Reachy Mini itself is hearing. YRobot Remote does not use this iPhone's microphone.")
        }
    }
    ...
    private func setMicUpload(_ enabled: Bool) async {
        ...
        do {
            ...
            micError = nil
            // Refresh status so the new input_enabled value is
            // visible in both this view and the Overview card.
            // Without this the toggle appears \"stuck\" because the
            // local status still shows the previous value.
