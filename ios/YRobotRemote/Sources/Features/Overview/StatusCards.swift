...
struct AudioCard: View {
    ...
    var body: some View {
        Section("Audio") {
            LabeledContent("Volume",
                value: "\(session.status?.status.audio.volume_percent ?? 0)%")
            HStack {
                Text("MIC enable")
                Spacer()
            ...
            if let err = session.status?.status.audio.volume_error {
    ...
    private func setMicUpload(_ enabled: Bool) async {
        ...
        do {
            ...
            micError = nil
            // Refresh status so the new input_enabled flag shows up
            // everywhere (Overview + Media tab).
