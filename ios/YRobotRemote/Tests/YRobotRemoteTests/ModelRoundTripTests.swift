...
final class ModelRoundTripTests: XCTestCase {
    ...
    // MARK: - Dual backend fixtures

    func testConversationBackendXIAOZHI() throws {
        try roundTrip(ConversationBackendEnvelope.self, fixture: "conversation_backend_xiaozhi")
    }
    func testConversationBackendQWEN() throws {
        try roundTrip(ConversationBackendEnvelope.self, fixture: "conversation_backend_qwen")
    }
    func testConversationBackendQWENFailed() throws {
        try roundTrip(ConversationBackendEnvelope.self, fixture: "conversation_backend_qwen_failed")
    }
    func testConversationVoice() throws {
        try roundTrip(ConversationVoiceEnvelope.self, fixture: "conversation_voice")
    }

    /// The QWEN chat fixture must round-trip through the conversation
    /// backend types so that the iOS UI can render both XIAOZHI and
    /// QWEN conversation histories without diverging parsers.
    func testQWENChatLogsRoundTrip() throws {
        try roundTrip(LogsResponse.self, fixture: "yrobot_logs_chat_qwen")
    }
