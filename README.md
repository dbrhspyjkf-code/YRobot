...
| Wooden motion | One 50 Hz thread owns the pose. Breathing and posture cross-fade; gaze and idle saccades use velocity-limited second-order trajectories. A slower body-yaw spring follows sustained physical head direction—including daemon-side face tracking—while leaving small head gestures independent | `motion.py` |
